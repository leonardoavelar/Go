...
func (t *contaCorrente) sacar (valor float64) {
	...
	if valor < 0 {
		...
		println("Transação cancelada")
		println("Valor:", fmt.Sprintf("%.2f", valor))
		println("O valor do saque é superior ao saldo da conta do cliente:", t.titular)	
...
func (t *contaCorrente) depositar (valor float64) {
	...
	if valor < 0 {
		...
		println("Transação realizado com sucesso")
		println("Valor:", fmt.Sprintf("%.2f", valor))
		println("Depósito realizado na conta do cliente:", t.titular)
